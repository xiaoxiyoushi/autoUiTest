...
public class EBFA04简历搜索_必须包含关键词_简历信息卡片 extends BoleBase {
    @Test(description = "校验大卡片")
    public void 搜索手机号_邮箱_公司名称_职位_学校_专业 ()throws IllegalAccessException, InstantiationException, InterruptedException {
        ...
        eBFA04Page.sendInputPlaceholder("请输入必须包含的关键词,用空格隔开",mustKeywords)
                .clickSpan("搜索").sleepForSeconds(3)
                .checkInform_Card(list,2);//在EBFA04简历搜索页面  校验小卡片

        list.add("新杨中学");
        list.add("英语");
        eBFA04Page.div_hover("张怡玲")//hover姓名
                .checkInform_Card(list,1).sleepForSeconds(2);//在EBFA04简历搜索页面  校验大卡片
